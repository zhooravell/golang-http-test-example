...
func TestCryptoCompareResource_BitCoinToUSDRateInvalidBaseURL(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
...
func TestNewCryptoCompareResource(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	r := NewCryptoCompareResource(server.Client())

	if reflect.TypeOf(r).String() != "*coins_rate.cryptoCompareResource" {
		t.Fail()
	}
}
