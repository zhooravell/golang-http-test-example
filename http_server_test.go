...
func TestGetRatesOneResourceFail(t *testing.T) {

	rateResources = make([]coins_rate.Resource, 2)
	rateResources[0] = &testResource{result: 10.5}
	rateResources[1] = &testResource{result: 0, err: errors.New("HTTP FAIL")}
	...
	if string(body) != "BitCoin to USD rate: 10.500000 $\n" {
		t.Fail()
	}
}
